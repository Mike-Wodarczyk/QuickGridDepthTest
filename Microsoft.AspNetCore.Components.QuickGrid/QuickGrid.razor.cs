...
public partial class QuickGrid<TGridItem> : IAsyncDisposable
{
    ...
    {
        Console.WriteLine("Setting Depth to " + depth);
        CurrentDepth = depth;
    }


    // Invoked by descendant columns at a special time during rendering
    public void AddColumn(ColumnBase<TGridItem> column, SortDirection? isDefaultSortDirection)
    {
		Console.WriteLine("AddColumn " + column.Title + " while _collectingColumns is " + _collectingColumns + " CurrentDepth " + CurrentDepth);
    ...
    {
        Console.WriteLine("StartCollectingColumns");
        CurrentDepth = 1;
        _columns.Clear();
    ...
    {
		Console.WriteLine("FinishCollectingColumns");
